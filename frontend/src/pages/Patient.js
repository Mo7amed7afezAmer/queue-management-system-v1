...
const Clinics = (props) => {
    ...
    */
    ...
    function showDeleteBox(e) {
        e.preventDefault();
        document.getElementById("formDeletepatient").value = e.target.id;
    ...
    */
    ...
    useEffect(() => {
        ...
        const fetchPatient = async (e) => {
            const result = await axios({
                url: `https://queue-management-system-v1.onrender.com/patient?pstatus=&pdate=&pclinic=`,
                method: `get`,
                headers: {
                "x-auth-token": localStorage.getItem("csrf_token")
                }
            });
            setPatientData(result.data.content);
            setLoading(true);
        }
    ...
    function addObject(e) {
        ...
        // a. collection data
        let pname = document.getElementById("aname").value;
        let ppassword = document.getElementById("apassword").value;
        let patage = document.getElementById("aage").value;

        // d. send data to server
        axios({
            url: "https://queue-management-system-v1.onrender.com/patient/add",
            method: "post",
            data: {
                name: pname,
                password: ppassword,
    ...
    function updateObject(e) {
        ...
        // d. send data to server
        axios({
            url: `https://queue-management-system-v1.onrender.com/clinic/update/${cid}`,
            method: "put",
            data: {
                name: cname,
                cstart: stime,
    ...
    function deleteObject(e) {
        ...
        // pervent reload page
        e.preventDefault();
        let pat_id = document.getElementById("formDeletepatient").value;

        console.log(pat_id)

        axios({
            url: `https://queue-management-system-v1.onrender.com/patient/delete/${pat_id}`,
            method: "delete",
            headers: {
                "x-auth-token": localStorage.getItem("csrf_token")
            }
    ...
    function filterDisplayData(e) {
        ...
        // variables
        let pstatus = document.getElementById("filterWithStatus").value,
            pdate = document.getElementById("filterWithDate").value,
            pclinic = document.getElementById("filterWithClinic").value;

        console.log(typeof pstatus)
        if (pstatus !== "1") {
            document.getElementById("filterWithClinic").setAttribute("disabled", true);
        } else {
            document.getElementById("filterWithClinic").removeAttribute("disabled");
        }

        axios({
            url: `https://queue-management-system-v1.onrender.com/patient?pstatus=${pstatus}&pdate=${pdate}&pclinic=${pclinic}`,
            method: `get`,
            headers: {
            "x-auth-token": localStorage.getItem("csrf_token")
            }
    ...
    useEffect(() => {
        ...
        const fetchClinic = async (e) => {
            ...
            setClinicData(result.data.content);
            setLoading(true);
        }
    ...
    return (
        <div className="patient-page pages">
                    ...
                    <div className="header-bottom">
                        <div className="filter-box">
                            <form onSubmit={ filterDisplayData }>
                                <select defaultValue="1" id="filterWithStatus">
                                    <option value=""> { langs("all") } </option>
                                    <option value="1"> { langs("available") } </option>
                                </select>
                                <select defaultValue="" id="filterWithClinic">
                                    <option value=""> { langs("all") } </option>
                                    { clinicData.length > 0 &&
                                            clinicData.map((el) => (
                                ...
                                <input type="date" id="filterWithDate" />
            ...
            <div className={`add-box ${actionType[0].complete || actionType[1].complete || actionType[2].complete  ? "open" : ""}`}>
                <div className={ `add ${actionType[0].complete ? "" : " d-none"}` }>
                    ...
                    <form className="form-add" onSubmit={ addObject }>
                        ...
                        <div className="form-group">
                            <label htmlFor="aage">{ langs("patient", "age") }</label>
                ...
                <div className={ `edit ${actionType[1].complete ? "" : " d-none"}` }>
                    ...
                    <form className="form-add" onSubmit={ updateObject }>
                        ...
                        <div className="form-group live-box">
                ...
                <div className={ `delete ${actionType[2].complete ? "" : " d-none"}` }>
                    <div className="box-header">
                        <p>{ langs("delete", "patient") }</p>
                        <i className="fa fa-close" onClick={ showDeleteBox }></i>
                    </div>
                    <form className="form-add">
                        <div className="form-group">
                            <h6>{ `${langs("confirmDeleteMessage")} ${langs("patient")}?` }</h6>
                        </div>
                        <div className="form-group btns">
                            <input type="hidden" id="formDeletepatient" />
